...
def cleanup():
    ...
    if local_ssh_agent and 'SSH_AGENT_PID' in os.environ:
        try:
            subprocess.run(['ssh-agent', '-k'], check=False, 
...
def start_xorg(disp, resolution):
    ...
    with open(log_file, 'w') as log, open(err_log, 'w') as err:
        cmd = [
            ...
            '-logfile', str(log_file), '-config', str(config_file),
            f':{disp}'
        ]
...
def start_novnc(vnc_port, web_port, disp):
    ...
    with open(log_file, 'w') as log:
        cmd = [
            ...
            '--vnc', f'localhost:{vnc_port}', '--listen', str(web_port)
        ]
...
def start_x11vnc(disp, vnc_port, password):
    ...
    with open(log_file, 'a') as log:
        cmd = [
            'x11vnc', '-display', f':{disp}', '-rfbport', str(vnc_port),
            '-xkb', '-repeat', '-skip_dups', '-forever', '-shared',
...
def main():
    ...
    parser.add_argument('-s', '--resolution', default='1920x1080',
    ...
    for var in list(os.environ.keys()):
        if re.match(r'^XDG|SESSION|^GTK|XKEYS|^WLS|WINDOWMANAGER|WAYLAND_DISPLAY|BROWSER', var):
    ...
    vnc_port = 5900 + disp
    web_port = 6080 + disp
    os.environ['XDG_RUNTIME_DIR'] = tempfile.mkdtemp(prefix=f"runtime-{os.getenv('USER')}-")
    ...
    with open(log_file, 'w') as log:
        p = subprocess.Popen(['lxsession', '-s', 'LXDE', '-e', 'LXDE'], 
    ...
    print(f"    http://localhost:{web_port}/vnc.html?resize=downscale&autoconnect=1&password={password}")
    print(f"or connect your VNC viewer to localhost:{vnc_port} with password {password}\n")

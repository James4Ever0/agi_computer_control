...
# since you would be doing these operations separately.
# 
# to measure the overall loss, you would consider merging two types of tensor into one.

# this model does not need to train the ViT transformer, and does not need to decode the vector embedding of the ViT output.
# 
# mouse actions could be tokenized if send x and y coords sequentially.
# 
# make x and y coords continuous by squeezing it into 4 separate floating point numbers.
# 
# change the codebase and speed up training in next revision.
...
# change of the encoding? how to encode these stuff?
# train similar actions together? to train the encoder?

# sparse -> embedding layer -> dense over every output to decode?
# 
# when doing sparse decoding, you can rank probability and sample.
# when doing dense embedding, you can add noise and remove determinism.

# you can even add random noise to embedding input (before going into softmax?) to make network active (non-dead).

# fourier transform?
# you encode these things by addition, using fourier transform to decode.
#

# for mouse coordinates:
# you encode different part of info into different frequencies
# then you decode the info using some decoder
# you may share the decoder, to ensure its integrety.

# mouse_emb (elementwise_product) mouse_fft

# [special_emb]
# [vit_emb]
# [mouse_emb] <- compound emb: emb_1*compound_sin_1 + emb_2*compound_sin_2+ emb_3*compound_sin_3+ emb_4*compound_sin_4-> (use 2 dense layers, or transformer?) multiple decoder (dense)
# [keyboard_emb]

# mouse use continual spatial sparse encoding from HTM, then pass through Linear layer.
# keyboard use conventional embedding

# for continual embedding, you output a single value, instead of sparse.
# if choices are few, make it sparse.

# so:
# mouse - continual, squeezed to single bits.
# special bits - sparse (fft or ifft?)
# keys - sparse

# how to encode multiple mouse embedding?
...
class ConsciousBlock(BaseModel):
    data_type:Union[Literal["image"],
                   Literal["HIDAction"]] # 2 bits, required
    special_token:Union[Literal["image_newline"],
                       Literal["image_end"],
                       Literal["action_end"], # change some of these bits into -torch.inf, so you won't have paradox like results.
                       None] = None # 4 bits
    image_data: Union[None, NDArray] = None # what is the shape of this image data? assume to be [3,224,224] (c h w) flattened
    ...
    def from_tensor(tensor:torch.Tensor):
        ...
        data_bits, special_bits, image_data, action_data = einops.unpack(tensor, [[s] for s in split_sizes], "*")
#         data_bits, special_bits, image_data, action_data = size_splits(tensor, split_sizes)
        ...
        if data_type == "image":
            ...
            special_token = ConsciousBase.special_tokens[int(torch.argmax(special_bits))]
            
            mConsciousBlock = ConsciousBlock(data_type=data_type, special_token=special_token, image_data=image_data)
        elif data_type == "HIDAction":
            special_bits[0:2] = -torch.inf
            special_token = ConsciousBase.special_tokens[int(torch.argmax(special_bits))]
            
            mConsciousBlock = ConsciousBlock(data_type=data_type, special_token=special_token, action_data=action_data)
        else:
...
#         print(ConsciousBase.split_sizes)
        data_bits, special_bits, image_data, action_data = einops.unpack(mTensorBase, [[s] for s in ConsciousBase.split_sizes], '*')
#         data_bits, special_bits, image_data, action_data = size_splits(mTensorBase, ConsciousBase.split_sizes)
        ...
        mTensor, _ = einops.pack((data_bits, special_bits, image_data, action_data), "*")

...
def place_cursor_to_screen(
    cursor: ImageClass, screen: ImageClass, position: tuple[int, int], red_on_grey: bool
):
    mask = cursor.copy()
    if red_on_grey:
...
def test(red_on_grey: bool):
    ...
    output_image = place_cursor_to_screen(
        cursor_image, screen_image, position, red_on_grey
    )
